...
// Sum quantity_sum where trayod = 36 and 18, joining tblproducts on sku
            ...
            $sumAll = ceil($sum36 / 36) + ceil($sum18 / 18);

            $tracks = ceil($sumAll / 64);

// Create the info box
$infoBox = new WidgetsBox('M&S Order Information', "<h5><p>Last due date: <strong>{$formattedDate}</strong> | Total Half Trays (trayod=36): <strong>{$sum36}</strong> | Total Metrics Trays (trayod=18): <strong>{$sum18}</strong> | Total Tracks: <strong>{$tracks}</strong></p></h5>");
...
$box->footer("<h5><p>Total Tracks: <strong>{$tracks}</strong></p></h5>");
//box options
//$box->removable();
$box->collapsable();
